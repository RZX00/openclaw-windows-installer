...
param(
    ...
    [string]$OutputDir,
    [string]$OutputName = "OpenClaw-Reach-Pack.exe",
...
$script:BuildRoot = Join-Path $env:TEMP ("openclaw-reach-pack-" + [guid]::NewGuid().ToString("N"))
...
function New-RunInstallCmd {
    ...
    $installLine = 'powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%~dp0\install-windows-reach-pack.ps1" -Locale "{0}" -InvokerRoot "%~dp0"' -f $Locale
    $lines = @(
        '@echo off',
        'setlocal',
        'cd /d "%~dp0"',
...
function New-EmbeddedOneClickExecutable {
    ...
    $payloadZipPath = Join-Path $script:BuildRoot "reach-pack-payload.zip"
    $stubExePath = Join-Path $script:BuildRoot "reach-pack-stub.exe"
...
function Build-ReachPack {
    ...
    $payloadRoot = Join-Path $stageDir "payload"
    $toolsRoot = Join-Path $payloadRoot "tools"
    $skillRoot = Join-Path $payloadRoot "skill\agent-reach"

    foreach ($path in @($downloadDir, $workRoot, $stageDir, $payloadRoot, $toolsRoot, $skillRoot)) {
        Ensure-Directory -Path $path
    }

    Write-Info ("Preparing offline Reach payload for {0}" -f $Architecture)
    ...
    $pythonMetadata = Prepare-PortablePython `
        ...
        -SkillDestinationRoot $skillRoot
    ...
    Save-JsonFile -Path (Join-Path $payloadRoot "reach-manifest.json") -Object ([pscustomobject]$manifest)

    if (-not $DryRun) {
        Copy-Item -LiteralPath (Join-Path $PSScriptRoot "install-windows-reach-pack.ps1") -Destination (Join-Path $stageDir "install-windows-reach-pack.ps1") -Force
    }
    [void](New-RunInstallCmd -StageDir $stageDir)

    if ($DryRun) {
        Write-Ok ("Dry run complete. Reach Pack would be written to: {0}" -f $outputExePath)
        return
    }

    Write-Info ("Building self-extracting Reach Pack: {0}" -f $outputExePath)
    New-EmbeddedOneClickExecutable -StageDir $stageDir -OutputExePath $outputExePath

    if (-not (Test-Path -LiteralPath $outputExePath)) {
        Write-Err ("Reach Pack was not produced: {0}" -f $outputExePath)
    }

    if ($KeepIntermediate) {
        $intermediateDir = Join-Path $effectiveOutputDir ("intermediate-reach-pack-" + (Get-Date -Format "yyyyMMdd-HHmmss"))
    ...
    Write-Ok ("Reach Pack created: {0}" -f $outputExePath)
}
